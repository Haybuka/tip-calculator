...
function People({people,setPeople}) {
  ...
  return (
    <aside>
      <h3 className='flex justify-between items-center mb-2 text-dark text-sm tracking-widest'>
        <p>Number of People</p>
        <p className="text-red-500">{people < 1 && "Can't be zero"}</p>
      </h3>
      <form className='relative '>
         <svg xmlns="http://www.w3.org/2000/svg" width="13" height="16" className='translate-x-2 absolute top-2/4 left-0 -translate-y-1/2'>
          ...
          <input value={people} onChange={e=> setPeople(e.target.value)} type='text' placeholder="0" className='text-dark font-bold outline-none py-2 px-2 text-right w-full bg-gray-100' />
      </form>
    </aside>
  )
}
